...
class _ActivityScreenState extends State<ActivityScreen> {
  ...
  void _load() {
    context.read<ActivityBloc>().add(
      ActivityEvent.loadExpenses(month: _selectedMonth, rangeStart: _dateRange?.start, rangeEnd: _dateRange?.end),
    );
  }
  ...
  Future<void> _pickDateRange() async {
    ...
    final picked = await showDateRangePicker(
      ...
      lastDate: now,
      initialDateRange: _dateRange ?? DateTimeRange(start: _selectedMonth, end: monthEnd.isAfter(now) ? now : monthEnd),
    );
  ...
  List<ActivityItem> _visibleItems(List<ActivityItem> items, String? userId) {
    ...
    if (_filter == _ActivityFilter.expenses) {
      ...
      filtered = items
          .where((i) => (i is SettlementItem && i.settlement.toUserId == userId) || i is IncomeItem || i is ManualDepositItem)
          .toList();
  ...
  ({double spent, double deposited, int spentCount, int depositCount}) _visibleTotals(List<ActivityItem> items, String? userId) {
    var spent = 0.0;
    var deposited = 0.0;
    var spentCount = 0;
    var depositCount = 0;
    ...
    for (final item in items) {
      switch (item) {
        case ExpenseItem(:final expense):
          spent += userId != null ? _userShare(userId, expense) : expense.amount;
          spentCount += 1;
    ...
    return (spent: spent, deposited: deposited, spentCount: spentCount, depositCount: depositCount);
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                _filterOption(label: 'Transfers only', value: _ActivityFilter.transfers, icon: Icons.compare_arrows_rounded),
                _filterOption(label: 'Audit entries', value: _ActivityFilter.audits, icon: Icons.fact_check_rounded),
                const SizedBox(height: 8),
                const Divider(),
                Align(
                  alignment: Alignment.centerLeft,
                  child: Text('Expense type', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
                ),
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(icon: const Icon(Icons.filter_list_rounded), tooltip: 'Filter', onPressed: _showFilterSheet),
        ],
      ),
      body: Column(
        children: [
              ...
              builder: (context, state) {
                return state.when(
                  ...
                  empty: () => const FinanceEmptyState(
                    ...
                    subtitle: 'Your expenses, income, and settlements will appear here.',
                  ),
                  error: (message) => FinanceEmptyState(icon: Icons.cloud_off_rounded, title: 'Could not load activity', subtitle: message),
                  loaded: (items) {
                    ...
                    if (filtered.isEmpty) {
                      return FinanceEmptyState(
                        ...
                        subtitle: 'Try another month or clear the active filters.',
                      ...
                      child: CustomScrollView(
                        slivers: [
                          for (int i = 0; i < sortedKeys.length; i++) ...[
                            ...
                            SliverPersistentHeader(pinned: true, delegate: _MonthHeaderDelegate(monthLabel: sortedKeys[i])),
                            SliverList(
                              delegate: SliverChildBuilderDelegate((context, index) {
                                final item = grouped[sortedKeys[i]]![index];
                                final card = switch (item) {
                                  ExpenseItem(:final expense) => _buildExpenseCard(expense, userId),
                                  SettlementItem(:final settlement, :final depositAccountName) => _buildSettlementCard(
                                    settlement,
                                    depositAccountName,
                                    userId,
                                  ),
                                  IncomeItem(:final income, :final accountName) => _buildIncomeCard(income, accountName),
                                  ManualDepositItem(:final deposit, :final accountName) => _buildManualDepositCard(deposit, accountName),
                                  TransferActivityItem(:final transfer, :final isIncoming, :final accountName, :final linkedAccountName) =>
                                    _buildTransferCard(transfer, isIncoming, accountName, linkedAccountName),
                                  BalanceCorrectionItem(:final deposit, :final accountName) => _buildBalanceCorrectionCard(
                                    deposit,
                                    accountName,
                                  ),
                                };
  ...
  Widget _buildMonthBar() {
    ...
    final isAtCurrent = _selectedMonth.month == now.month && _selectedMonth.year == now.year;

    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
      child: Row(
        children: [
          IconButton(icon: const Icon(Icons.chevron_left_rounded), onPressed: _prevMonth, tooltip: 'Previous month'),
          GestureDetector(
          ...
          IconButton(
            icon: Icon(Icons.chevron_right_rounded, color: isAtCurrent ? Theme.of(context).disabledColor : null),
            onPressed: isAtCurrent ? null : _nextMonth,
            tooltip: 'Next month',
          ),
          const Spacer(),
          IconButton(onPressed: _pickDateRange, icon: const Icon(Icons.calendar_month_rounded), tooltip: 'Pick date range'),
        ],
      ),
    );
  }
  ...
  Widget _buildSettlementCard(SettlementEntity settlement, String? depositAccountName, String? userId) {
    final s = settlement;
                ...
                secondChild: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Icon(income.source.icon, size: 16, color: Theme.of(context).colorScheme.onSurfaceVariant),
                          const SizedBox(width: 6),
                          Text(income.source.label, style: TextStyle(fontSize: 13, color: Theme.of(context).colorScheme.onSurfaceVariant)),
                        ],
                      ),
                      if (accountName != null) ...[
                        const SizedBox(height: 6),
  ...
  Widget _buildManualDepositCard(ManualDepositDto deposit, String? accountName) {
          ...
          child: Column(
            ...
            children: [
              _cardHeader(
                ...
                iconColor: Colors.green,
                title: deposit.description.isEmpty ? 'Manual deposit' : deposit.description,
              ...
              AnimatedCrossFade(
                ...
                secondChild: Padding(
                  ...
                  child: _accountDetailLine(
                    ...
                    text: accountName == null ? 'Manual deposit' : 'Deposited to $accountName',
                  ),
                ),
                crossFadeState: isExpanded ? CrossFadeState.showSecond : CrossFadeState.showFirst,
                duration: const Duration(milliseconds: 200),
  ...
  Widget _buildTransferCard(TransferDto transfer, bool isIncoming, String? accountName, String? linkedAccountName) {
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              _cardHeader(
                icon: isIncoming ? Icons.call_received_rounded : Icons.call_made_rounded,
                iconColor: Colors.indigo,
                title: transfer.description.isEmpty ? title : transfer.description,
              ...
              AnimatedCrossFade(
                ...
                secondChild: Padding(
                  padding: const EdgeInsets.only(top: 12),
                  child: _accountDetailLine(icon: Icons.compare_arrows_rounded, text: detail),
                ),
                crossFadeState: isExpanded ? CrossFadeState.showSecond : CrossFadeState.showFirst,
                duration: const Duration(milliseconds: 200),
              ),
  ...
  Widget _buildBalanceCorrectionCard(ManualDepositDto deposit, String? accountName) {
    final isExpanded = _expandedIds.contains(deposit.id);
          ...
          child: Column(
            ...
            children: [
              _cardHeader(
                ...
                title: deposit.description.isEmpty ? 'Balance correction' : deposit.description,
                date: deposit.createdAt,
                amount: deposit.amount,
                badge: 'AUDIT',
                ...
                secondChild: Padding(
                  ...
                  child: Column(
                    children: [
                      _accountDetailLine(
                        icon: Icons.account_balance_rounded,
                        text: accountName == null ? 'Balance correction' : 'Corrected $accountName',
                      ),
                      if (previous != null && updated != null) ...[
                        const SizedBox(height: 6),
                        _accountDetailLine(
                          icon: Icons.edit_note_rounded,
                          text: '${formatIndianRupee(previous)} to ${formatIndianRupee(updated)}',
                        ),
                      ],
                    ],
                  ),
          ...
          child: Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 4,
                children: [
                  Text(DateFormat('dd MMM yyyy').format(date), style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                  if (isExpanded) ...[
                    Text(
                  ...
                  if (category != null && category != ExpenseCategory.other.name) _buildCategoryBadge(category, Theme.of(context)),
                ],
              ),
            ],
          ),
...
class _ActivityMetricCard extends StatelessWidget {
  ...
  const _ActivityMetricCard({required this.label, required this.value, required this.subtext, required this.icon, required this.color});
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(subtext, style: TextStyle(fontSize: 11, color: Theme.of(context).colorScheme.onSurfaceVariant)),
          ],
        ),
      ),
    );
