  ...
  Future<void> _load() async {
    try {
      ...
      for (final a in accounts) {
        final snap = await repo.getSnapshot(a.id, now.year, now.month);
        snapshots[a.id] = snap;
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : _savingsAccounts.isEmpty
              ? const Center(child: Text('No savings accounts. Mark an account as savings first.'))
              : RefreshIndicator(
                  onRefresh: _load,
                  child: ListView(
                    ...
                    children: [
                      ...
                      ..._savingsAccounts.map((a) => Padding(
                        padding: const EdgeInsets.only(bottom: 8),
                        child: _buildAccountCard(a),
                      )),
                      if (_timeline.isNotEmpty) ...[
                        const SizedBox(height: 12),
                        const Text('Monthly History', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('This Month', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
              ...
              children: [
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Goal', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 13)),
                      const SizedBox(height: 4),
                      Text(formatIndianRupee(totalGoal), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ],
                  ),
                ),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text('Saved', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 13)),
                      const SizedBox(height: 4),
                      Text(formatIndianRupee(totalSaved), style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: barColor)),
                    ],
                  ),
                ),
              ],
            ...
            const SizedBox(height: 8),
            Text('${percent.toStringAsFixed(1)}% achieved', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 13)),
          ],
        ),
      ),
    );
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(formatIndianRupee(account.currentBalance), style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 13)),
                      ],
                    ),
                  ),
                  Text('${percent.toStringAsFixed(0)}%', style: TextStyle(fontWeight: FontWeight.w600, color: barColor)),
                ],
              ),
              if (goal > 0) ...[
                const SizedBox(height: 12),
                ...
                const SizedBox(height: 4),
                Text('${formatIndianRupee(saved)} / ${formatIndianRupee(goal)}', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12)),
              ],
            ],
          ),
        ),
    ...
    return Card(
      ...
      child: ListTile(
        leading: CircleAvatar(
          ...
          child: Icon(isNegative ? Icons.warning_amber_rounded : Icons.check_circle_outline_rounded, color: isNegative ? Colors.red : Colors.green, size: 20),
        ),
        title: Text(label, style: TextStyle(fontWeight: FontWeight.w600, color: isNegative ? Colors.red.shade700 : null)),
        trailing: Text(
