...
void main() {
  ...
  group('computeCurrentMonth', () {
    test('creates new snapshot when no existing snapshot', () async {
      when(() => mockLocal.getSnapshot('acct-1', 2026, 7)).thenAnswer((_) async => null);
      when(() => mockLocal.getSnapshotsByAccount('acct-1')).thenAnswer((_) async => []);
      ...
      when(() => mockAccountLocal.getAccounts()).thenAnswer((_) async => [
        AccountDto(id: 'acct-1', userId: 'user-a', accountName: 'Savings', accountType: AccountType.bank, currentBalance: 5000, createdAt: now, updatedAt: now, isSavings: true),
      ]);

      final result = await repository.computeCurrentMonth('acct-1', 5000.0, 10000.0);
      ...
      verify(() => mockLocal.saveSnapshot(any())).called(1);
    });
    ...
    test('updates existing snapshot with new balance', () async {
      ...
      when(() => mockLocal.getSnapshot('acct-1', 2026, 7)).thenAnswer((_) async => existingDto);
      when(() => mockLocal.saveSnapshot(any())).thenAnswer((_) async {});
      when(() => mockRemote.saveSnapshot(any())).thenAnswer((_) async {});

      final result = await repository.computeCurrentMonth('acct-1', 2000.0, 10000.0);
    ...
    test('computes negative saved amount when balance is below opening', () async {
      final existingDto = MonthlySavingDto(
        id: 'ms-1',
        accountId: 'acct-1',
        year: 2026,
  ...
  group('getTotalSavedYearToDate', () {
    test('sums savedAmount for current year across all savings accounts', () async {
      ...
      when(() => mockAccountLocal.getAccounts()).thenAnswer((_) async => [savingsAccountDto]);
      ...
      when(() => mockLocal.getSnapshotsByAccount('acct-savings-1')).thenAnswer((_) async => [janDto, julDto]);
    ...
    test('skips snapshots from previous years', () async {
      ...
      when(() => mockLocal.getSnapshotsByAccount('acct-savings-1')).thenAnswer((_) async => [oldDto]);
  ...
  group('getAccountHistory', () {
    test('returns snapshots sorted by year/month descending', () async {
      ...
      when(() => mockLocal.getSnapshotsByAccount('acct-1')).thenAnswer((_) async => [julDto, augDto]);
  ...
  group('finalizeMonth', () {
    test('marks snapshot as completed', () async {
      ...
      when(() => mockLocal.getSnapshot('acct-1', 2026, 7)).thenAnswer((_) async => dto);
      when(() => mockLocal.saveSnapshot(any())).thenAnswer((_) async {});
      when(() => mockRemote.saveSnapshot(any())).thenAnswer((_) async {});

      await repository.finalizeMonth('acct-1', 2026, 7);

      verify(() => mockLocal.saveSnapshot(any(that: predicate<MonthlySavingDto>((s) => s.isCompleted)))).called(1);
    });
